var urlhistory = window.localStorage.getItem("myhistory") && window.localStorage.getItem("myhistory").split(",") || new Array;
...
if(urlhistory[urlhistory.length-1] != window.location.href){
	...
	if(window.location.href.indexOf("im/msgcount") > -1){
		//如果im未读消息数页面，不记录访问历史
	}else{
		//记录访问历史
		urlhistory.push(window.location.href);
