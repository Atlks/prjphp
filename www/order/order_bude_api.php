...
if ($filter) {
    $sql = "select * from order_bill WHERE pay_time is not null and app_id=$merid and bude='y'";
} else {
...
if ($filter) {
    $sql = "select * from order_bill WHERE   bude='y' and pay_time is not null and app_id=$merid ORDER BY id DESC limit $limit OFFSET $page";
} else {
